import { Status, Level } from './status.enum';

export const Utils = {
    getByEnum,
    getLevelByEnum,
    getReasonByEventCode,
};

function getByEnum(_status) {
    try {
        let enumValue = 'transparent';
        if (Status.hasOwnProperty(_status)) {
            enumValue = Status[_status];
        }
        return enumValue;
    } catch (error) {
        console.log(error)
...
function getLevelByEnum(_level) {
    try {
        let enumValue = 'transparent';
        if (Level.hasOwnProperty(_level)) {
            enumValue = Level[_level];
        }
        return enumValue;
    } catch (error) {
        console.log(error)
...
function getReasonByEventCode(eventCode) {
