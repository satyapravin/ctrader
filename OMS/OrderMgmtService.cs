...
namespace OMS
{
    ...
    public class OrderMgmtService : IEmbeddedService
    {
        ...
        private static readonly ILog Log = log4net.LogManager.GetLogger(typeof(OrderMgmtService));
        private OnOrder orderHandler = null;
        private ExchangeService.Exchange svc = null;
        private readonly BlockingCollection<BitmexSocketDataMessage<IEnumerable<OrderDto>>> queue = new BlockingCollection<BitmexSocketDataMessage<IEnumerable<OrderDto>>>();
        private readonly BlockingCollection<OrderDto> clientNotificationQ = new BlockingCollection<OrderDto>();
        private readonly Dictionary<string, MyOrder> oms_cache = new Dictionary<string, MyOrder>();
        private readonly Dictionary<string, MyOrder> oms_pending_cache = new Dictionary<string, MyOrder>();
        private readonly Dictionary<string, MyOrder> oms_cancel_cache = new Dictionary<string, MyOrder>();
        private readonly Dictionary<string, MyOrder> oms_amend_cache = new Dictionary<string, MyOrder>();
        private readonly Dictionary<string, MyOrder> oms_processed_cache = new Dictionary<string, MyOrder>();
        #endregion
        ...
        internal async void AmendOrder(MyOrder req, decimal price)
        {
            ...
            {
                ClientOrderID = req.Symbol + DateTime.Now.Ticks.ToString(),
            ...
            {
                ClOrdID = amend.ClientOrderID,
                OrigClOrdID = amend.ChildOrder.ClientOrderID,
                Price = amend.Price
            };
            ...
            {
                Log.Error("amend resulted in exception", e);
                lock (oms_cache)
                {
        ...
        internal async void NewOrder(MyOrder req)
        {
            if (req.Status == OrderStateIdentifier.UNCONFIRMED)
            {
                ...
                {
                    Log.Error("New resulted in exception", e);
                    lock (oms_cache)
                    {
        ...
        internal async void CancelOrder(MyOrder req)
        {
            ...
            {
                Log.Error("Cancel resulted in exception", e);
                lock (oms_cache)
                {
        ...
        public void RegisterHandler(OnOrder handler)
        {
        ...
        private void OnStart()
        {
            ...
            {
                Log.Fatal("OMS Thread crashed", e);
            }
        }
        ...
        private void OnClientNotification()
        {
            foreach (var d in clientNotificationQ.GetConsumingEnumerable(CancellationToken.None))
            {
                if (orderHandler != null)
                {
                    ...
                    {
                        Log.Error("Client order notification handler excepted", e);
                    }
                }
            }
        }
